...
Only the Next routes save an exact input together with its app's Mode and auto-send. If VoiceInk++ can't verify that input, it shows an error instead of pasting somewhere else. The [destination guide](RECORDING_DESTINATIONS.md) has the full contract.
