...
- On `recordingStoppedPreservingPlayback`, it balances/clears the same dictation ownership without sending any play or pause command. Current VoiceInk++ source emits this event for the recording-time double-press clipboard finish; installing the companion alone does not add a recording gesture to an older app.
