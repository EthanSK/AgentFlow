...
9. If Agentic Mouse is installed, check its documented seek, volume, speed and tab-history controls separately. Its hardware configuration is not bundled here. Current VoiceInk++ source emits the preserving-playback notification for its recording-time clipboard finish; verify the installed app version before claiming that gesture works on the user's Mac.
